...
public class SerialNumberService {
     ...
     */
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    @OperationLog(moduleName="序列号", operationType=OperationType.BATCH_ADD)
    public int batAddSerialNumber(String materialCode, String serialNumberPrefix, Integer batAddTotal, String remark)throws Exception {
        ...
        try {
            if (StringUtil.isNotEmpty(materialCode)) {
                ...
                LogContentProvider.setContent(BusinessConstants.LOG_OPERATION_TYPE_BATCH_ADD + batAddTotal + BusinessConstants.LOG_DATA_UNIT);
            }
        } catch (Exception e) {
            JshException.writeFail(logger, e);
        }
